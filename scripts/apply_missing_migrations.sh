...
puts "🔧 Отменяю fake для миграции 0004 и применяю ее реально..."
spawn ssh -o StrictHostKeyChecking=no $user@$host "cd $backend_dir && sudo -u www-data bash -c 'source venv/bin/activate && python3 manage.py migrate core 0003 --fake && python3 manage.py migrate core 0004 2>&1'"
...
puts "🔍 Проверяю, создалась ли таблица menu_item_images..."
