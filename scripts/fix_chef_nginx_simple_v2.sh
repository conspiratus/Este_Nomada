...
# Простое исправление nginx для /chef/ (версия 2)
...
puts "=========================================="
puts "Простое исправление nginx для /chef/"
puts "=========================================="
...
send "cat > /tmp/chef_simple_v2.conf << 'EOFCONF'
...
# Вставляем после строки 139 (закрывающая скобка location /api/)
puts "\n📝 Добавление location после /api/ (строка 139)..."
send "sudo sed -i '139r /tmp/chef_simple_v2.conf' /etc/nginx/sites-available/estenomada\r"
expect "administrator@*"
...
send "sudo sed -n '129,200p' /etc/nginx/sites-available/estenomada | head -40\r"
expect "administrator@*"
