...
# Простое исправление nginx для /chef/
...
send "cat > /tmp/chef_simple.conf << 'EOFCONF'
...
# Вставляем перед location / (простой поиск)
send "sudo sed -i '/location \\/ {/r /tmp/chef_simple.conf' /etc/nginx/sites-available/estenomada\r"
expect "administrator@*"
